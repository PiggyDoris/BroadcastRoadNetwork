...
void Client::getRandQueryPoint(string inputDir)
{
	int lineNumber = 0;
	int attributeFlag = 0;
...
void Client::listenBroadcastIndex(string inputDir, string inputDir1, string inputDir2, string inputDir3, string inputDir4)
{
	int nowRegionId;
	Region* nowRegion;
	int lineNumber = 0;
...
void Client::locateQueryPointRegion()
{
	for (int i = 0; i < allRegionList.size(); i++)
	{
...
void Client::listenBroadcastData(string inputDir)
{
	//get .bnnnX
	char line[SIZE];
	fstream fin;
...
void Client::computeCandidateRegion()
{
	computeUpperBound();
	computeLowerBound();
	...
	if (firstComputeCandidateFlag == true)
	{
		for (int i = 0; i < lowerBoundList.size(); i++)
		{
			...
			if (lowerBoundList.at(i)->getUpperBound() != -1 && lowerBoundList.at(i)->getUpperBound() < upperBound/* && allRegionNodeAmount.at(regDestinationRegionId) != 0*/)
	...
	if (maxDelta < 0 || minDelta < 0)
	{
		...
		for (int i = 0; i < queryRRMinDistList.size(); i++)
		{
			...
			if (i == queryPoint->getNodeRegionId())
				...
				regUpperBound->setUpperBound(maxDelta + queryRRMinDistList.at(i) + allRegionMaxdistList.at(i));
			upperBoundList.push_back(regUpperBound);
		}
	...
	for (int i = 0; i < recievedBNNRegionList.size(); i++)
	{
		...
		for (int j = 0; j < lowerBoundList.size(); j++)
		{
			if (lowerBoundList.at(j)->getQueryRegionId() == queryPoint->getNodeRegionId() && lowerBoundList.at(j)->getDestinationRegionId() == recievedBNNRegionList.at(i))
			{
				...
				lowerBoundList.at(j)->setUpperBound(regLowerBound);
				break;
			}
		}
...
void Client::computeKNNAnswer(string inputDir, string inputDir1, string inputDir2, string inputDir3, string inputDir4, string inputDir5, string inputDir6, int queryAmountK, int startRegionId)
{
	...
	while (processFlag == 1 || candidateRegionToListenList.size() != 0)
	{
		...
		listenBroadcastIndex(inputDir1, inputDir2, inputDir3, inputDir4, inputDir5);
		if (processFlag == 1)
		{
		...
		if (this->startRegionId - 1 == queryPoint->getNodeRegionId())
		{
			...
			if (nowRecievedNodeAmount >= queryAmountK)
			{
		...
		else if (queryRegionRecievedFlag == true && nowRecievedNodeAmount >= queryAmountK)
		{
			computeToQueryDijkstra();
			if (candidateRegionToListenList.size() == 0)
				queryFinishedFlag = true;
...
void Client::computeToQueryDijkstra()
{
...
vector<int> Client::nonRepeatRandAndSort(int total, int needNum)
{
	vector<int> card;
...
void Client::setPacketSize(int packetSize)
{
...
void Client::setStartRegionId(int startRegionId)
{
	this->startRegionId = startRegionId;
}

int Client::getStartIndexId()
{
	return startRegionId;
}

void Client::addRecievedBytes(int inputSize)
{
	recievedBytes = recievedBytes + inputSize;
	//cout << "Now Recieved Bytes : " << recievedBytes << endl;
}

void Client::computePacketSize()
{
	recievedPacketNum = recievedPacketNum + ceil((double)recievedBytes / (double)sizePerPacket);
}

void Client::computeRRMinDist()
{
}

vector<int> Client::findBorderNodeRegion(string nodeName)
{
...
double Client::computeSectionDistance(double x_lat, double x_lng, double y_lat, double y_lng)
{
	double x_radLat = 0;
	double y_radLat = 0;
	double a = 0, b = 0;
...
double Client::rad(double d)
{
	return d * pi / 180.0;
}

void Client::setQueryAmountK(int queryAmountK)
{
	this->queryAmountK = queryAmountK;
}

int Client::getQueryAmountK()
{
	return queryAmountK;
}

int Client::checkBorderNodeExistByName(vector<BorderNode*> borderNodeList, string nodeName)
{
...
void Client::getAllRegionBorderNodeLine()
{
	Region* regQueryRegion = allRegionList.at(queryPoint->getNodeRegionId());
...
void Client::computePerIndexSize(string inputDir, string inputDir1, string inputDir2, string inputDir3, string inputDir4, vector<int> bNNIndList)
{
	char line[SIZE];
	fstream fin;
	int lineNumber = 0;
...
void Client::computePerDataSize(string inputDir)
{
	char line[SIZE];
	fstream fin;
	char *pch;
...
int Client::computeIndexLatencySize(string inputDir, string inputDir1, int startRId, int endRId)
{
...
int Client::computeDataLatencySize(string inputDir, int startRId, int endRId)
{
	int nowLatencySize = 0;
	bool broadcastRepeatFlag = false;
	int regEndRid;
...
void Client::setQueryPoint(Node* queryPoint)
{
	this->queryPoint = queryPoint;
}

Node* Client::getQueryPoint()
{
	return queryPoint;
}

int Client::getTuningDataSize()
{
	return tuningDataSize;
}

vector<Node*> Client::getQueryKNNList()
{
	return queryKNNList;
}

int Client::getLatencyDataSize()
{
	return latencyDataSize;
}

int Client::getBroadcastCycleSize()
{
